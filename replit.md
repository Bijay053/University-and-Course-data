...
Keep one logical course identity with all verified campus offerings underneath it, not separate courses per campus. University/course APIs are consumed by another portal for student-profile university applications: expose one course ID with all its locations, without requiring fee selection. Location-specific fee display and selection belong only in Course Search, not application selection or ordinary course-management screens. Preserve existing API compatibility. Never invent locations, merge distinct awards, mix fee years or billing periods, or apply bursary-discounted prices as gross tuition. Ambiguous mappings stay pending for review, and preparation must not approve courses automatically. Do not display manual fee-choice radio buttons.

Review loading must not create campus-specific course duplicates. Approval preserves verified location-specific fees under the same parent course, including when processing legacy campus-scoped staging evidence. This is explicit reviewer approval, not unattended system publishing. Unverifiable evidence stays pending with a reason. Existing published duplicates must not be destructively merged without reference-preserving reconciliation and explicit authorization.
...
- **Database and environment**: `.env` contains general non-secret application configuration and `.release.env` contains only immutable release metadata. Production `DATABASE_URL` is generated only in root-owned `/etc/university-portal/database.env`, loaded last by both services. Its source is the exact stack-configured RDS-managed Secrets Manager ARN; refresh it only with `deploy/refresh_database_credentials_via_secrets_manager.py`, never by copying a password or URL through a shell, SSM parameter, or deployment argv. The fixed SSM document atomically updates the file, restarts API and Celery, and verifies TLS `SELECT 1`, rolling back on failure. Use `app.database.AsyncSessionLocal` for production queries without printing configuration. The AWS host does not have the retired server's local `postgres` OS account.
