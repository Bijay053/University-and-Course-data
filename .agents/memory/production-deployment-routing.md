...
**How to apply:** Fence the observed ledger, inventory every intermediate
schema contract, and reject replay of migrations containing data rewrites when
their schema is already materialized. Back up first, pause consumers, run only
the reviewed additive revision transactionally, and advance the ledger in that
same transaction only if all ancestors are proven present.
