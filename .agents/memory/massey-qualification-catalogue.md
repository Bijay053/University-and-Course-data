...
**How to apply:** Seed the dedicated index, force its child paths as candidates, keep the index listing-only, and apply a final detail-path allow-list. Use static proxy extraction, suppress the unhelpful per-course browser pass, and pin authoritative central tuition plus degree-level English defaults.