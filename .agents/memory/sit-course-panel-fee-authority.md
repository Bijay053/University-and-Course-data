...
**How to apply:** Central tuition may override local amounts only after an exact programme match with a non-empty tuition value. A required YAML schedule replaces stale legacy and request-level fee-page values. Central prefetch must fall back to the orchestrator's already-loaded config when task-local context is empty. A course absent from the populated international schedule is not eligible for staging; a schedule fetch failure remains retryable.