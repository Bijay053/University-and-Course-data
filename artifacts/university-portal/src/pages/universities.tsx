    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <>
            {/* Table header */}
            <div className="hidden md:grid grid-cols-[40px_2.5fr_1.5fr_80px_100px_2fr_auto] items-center px-5 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100 bg-gray-50/40">
              <span>SN.</span>
            ...
            <div className="divide-y divide-gray-50">
              {universities.map((uni, idx) => {
                ...
                return (
                  <div key={uni.id}
                    className="group grid md:grid-cols-[40px_2.5fr_1.5fr_80px_100px_2fr_auto] items-center px-5 py-3.5 hover:bg-blue-50/40 transition-colors">
                    ...
                    <div className="hidden md:flex items-center">
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); toggleFeatured(uni.id, !!(uni as { featured?: boolean }).featured); }}
                        title={(uni as { featured?: boolean }).featured ? "Featured — click to disable" : "Mark as Featured"}
                        className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-1 rounded-full border transition-all cursor-pointer disabled:opacity-50 ${
                          (uni as { featured?: boolean }).featured
                            ? "bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100"
                            : "bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100 hover:text-gray-600"
                        }`}
                      >
                        <Star className={`w-3 h-3 ${(uni as { featured?: boolean }).featured ? "fill-amber-500 text-amber-500" : ""}`} />
                        {(uni as { featured?: boolean }).featured ? "ON" : "OFF"}
                      </button>
                    </div>
