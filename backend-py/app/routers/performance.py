...
async def performance_summary(
    ...
    row = _row((await db.execute(text("""
        SELECT
            ...
            SUM(scrape_do_render_calls)                               AS total_sd_render_calls,
            SUM(scrape_do_static_calls)                               AS total_sd_static_calls,
            SUM(scrape_do_cost_usd)                                   AS total_sd_cost,
    ...
    return {
        ...
        "total_scrape_do_render_calls": _i(row.get("total_sd_render_calls")),
        "total_scrape_do_static_calls": _i(row.get("total_sd_static_calls")),
        "total_scrape_do_cost_usd": _f(row.get("total_sd_cost"), 4),
...
async def performance_trend(
    ...
    rows = (await db.execute(text("""
        SELECT
            ...
            SUM(scrape_do_render_calls)                                        AS sd_render_calls,
            SUM(scrape_do_static_calls)                                        AS sd_static_calls,
            SUM(scrape_do_cost_usd)                                            AS sd_cost,
    ...
    for r in rows:
        ...
        months_out.append({
            ...
            "scrape_do_render_calls": _i(r.get("sd_render_calls")),
            "scrape_do_static_calls": _i(r.get("sd_static_calls")),
            "scrape_do_cost_usd": _f(r.get("sd_cost"), 4),
