...
async def run_scrape(db: AsyncSession, runtime_job_id: str) -> dict:
    ...
    try:
        ...
        # path gather runs, those URLs are already cached â it awaits the same
        ...
        # cannot affect a single shadow run â both paths see the same cache hit).
        # Cross-run non-determinism (different values across the 5-run streak)
        # is expected and acceptable: each run's oldânew diff will still be
        ...
        if _shadow_on(uni_id):
            ...
            try:
                ...
                # New path transforms the already-extracted old results â no re-fetch,
