...
async def _run_claimed_scrape(db: AsyncSession, job, _verification=None) -> dict:
    ...
    try:
                ...
                try:
                    ...
                    if _verification and (job.request_payload or {}).get("courseReport"):
                        ...
                        _verified_programmes = (
                            _report_verification.get("verified_programmes") or {}
                        )
                        _proof = _verified_programmes.get(r.get("url") or "")
                        if _proof:
                            payload["_verified_report_programme"] = _proof
                        # Exact direct-page delivery proof is generated by the
                        # report endpoint from a structured official label, not
                        # from the user's assertion. It may correct a broad-text
                        # false Online result while preserving all global guards
                        # for actual ODL/online pages.
                        _verified_delivery = (
                            _report_verification.get("verified_delivery_modes") or {}
                        ).get(r.get("url") or "")
                        if (
                            isinstance(_verified_delivery, dict)
                            and _verified_delivery.get("mode") in {"On Campus", "Blended"}
                            and _verified_delivery.get("method") in {
                            ...
                            payload["study_mode"] = _verified_delivery["mode"]
                            payload["online_only"] = False
                            payload["online_only_utas"] = False
                            _stage_evidence.append({
                                "field_key": "study_mode",
                                "value": _verified_delivery["mode"],
                                "method": "course_report:verified_delivery",
                                "confidence": 0.95,
                                "snippet": _verified_delivery.get("evidence", ""),
        ...
        try:
            ...
            # Rollback is mandatory: a DB error caught here leaves the session
            # in "pending rollback" state; without an explicit rollback every
            # subsequent db.X() call raises "Can't reconnect until invalid
            # transaction is rolled back" and kills the finalization step.
            try:
                await db.rollback()
            except Exception:  # noqa: BLE001
                pass
        ...
        try:
            ...
            # Rollback is mandatory — see comment in _dq_exc handler above.
            try:
                await db.rollback()
            except Exception:  # noqa: BLE001
                pass
