...
IMPORTANT â this module must NOT re-fetch course URLs.
...
Correct pattern (Option A â single-process, single fetch):
  1. Old path fetches + extracts â produces result dict (payload, evidence, url)
...
----------------------------------------------------
Phase 1 (harness validation â DONE, 5-run streak achieved 2026-04-30):
...
Phase 2 (domestic_only YAML validation â CURRENT):
  ...
  surfaces 14+ staging_disagreements â one per domestic-only course.
  Target: 5 consecutive clean runs â proceed to shared-code cutover.

Phase 3 (shared-code cutover â pending):
...
async def extract_new_path(
    ...
    *,
    ...
    Args:
        ...
        uni_id:     University ID â used to select per-uni transformations below.
    ...
    # ââ Per-uni transformations ââââââââââââââââââââââââââââââââââââââââââââââââ

    if uni_id == 41:  # ACAP â Phase 2: domestic_only YAML config validation
...
def _apply_acap_domestic_only(new_result: dict[str, Any]) -> None:
    ...
    Behaviour:
        enabled=True  (acap.yaml current value):
            ...
            No change to new_result â this is a no-op. Shadow stays clean.
        ...
        enabled=False (misconfiguration guard):
            ...
            in the shadow report â catches accidental config regressions.
    ...
    try:
        ...
        if not enabled:
            # YAML says domestic_only detection is OFF for ACAP â clear any flag
        ...
        # enabled=True: no-op â deepcopy already preserved the flag
