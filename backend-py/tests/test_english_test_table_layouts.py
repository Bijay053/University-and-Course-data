...
# ─────────────────────────────────────────────────────────────────────
# Task 60 — _parse_equivalence_table must capture PTE overall even when
# <th colspan="3">Pearson Test of English</th> spans Overall, Listening,
# and Writing sub-columns.  Verifies that the colspan expansion handles
# three or more sub-skill columns, not just two.
# ─────────────────────────────────────────────────────────────────────


def test_parse_equivalence_table_pearson_colspan3_captures_pte_overall():
    """_parse_equivalence_table must return the correct PTE overall score
    when the Pearson group header spans three sub-skill columns (Overall +
    Listening + Writing) via ``colspan="3"``."""
    from bs4 import BeautifulSoup
